...
#import "TXChineseToPinyin.h"
static NSArray *lqq_resultArray = nil;
@interface TXSortByNameCore ()
@property (strong,nonatomic)NSArray *resultArray;
@property (strong,nonatomic)NSMutableDictionary *sourceDic;
@end
@implementation TXSortByNameCore
+ (NSArray*)sortDataByFirstLetterWithArray:(NSArray*)sourceArray isIncludeKeys:(BOOL)isKey{
    
    //按名字的首字母分类
    NSArray *arr = [self sortDataByFirstLetterWithArray:sourceArray];
    if (isKey) {
...
+ (NSArray*)sortDataByFirstLetterWithArray:(NSArray*)sourceArray {
    ...
    for (NSString *str in sourceArray) {
        ...
        //不是的话转换成拼音,并获取首字母
        if (!firstLetter) {
            firstLetter = [NSString stringWithFormat:@"%c",[TXChineseToPinyin sortSectionTitle:str]];
        }
